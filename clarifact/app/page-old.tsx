...
// --- COMPONENT: GLOBE HEATMAP WITH RADIUS CIRCLES ---
const GlobeHeatmapVisualization = ({ heatmap }) => {
  // Country coordinates (latitude, longitude) - Mercator projection
  const countryCoords = {
  ...
  const getRiskColor = (risk) => {
    if (risk > 0.8) return { color: '#DC2626', label: 'Critical' };
    if (risk > 0.6) return { color: '#EA580C', label: 'High' };
    if (risk > 0.4) return { color: '#FBBF24', label: 'Medium' };
    if (risk > 0.2) return { color: '#84CC16', label: 'Low' };
    return { color: '#10B981', label: 'Safe' };
  };

  // Convert lat/lng to pixel coordinates for globe.png projection
  const latLngToPixel = (lat, lng, containerWidth, containerHeight) => {
    // Assuming globe.png uses equirectangular projection (standard)
  ...
  React.useEffect(() => {
    if (containerRef.current) {
      ...
      setImageDimensions({ width, height: width * 0.5 });
    }
  }, []);

  return (
    <div className="w-full flex flex-col items-center justify-center bg-gradient-to-b from-blue-950/30 to-slate-950 p-4 rounded-lg border border-slate-700/50">
      {/* Globe image container with risk circles overlay */}
      <div 
        ref={containerRef}
      ...
      >
        ...
        >
          ...
          {/* Risk circles positioned on globe */}
          {heatmap && heatmap.map((item) => {
            const coords = countryCoords[item.country];
            if (!coords) return null;
            
            const riskValue = item.risk || 0;
            const riskInfo = getRiskColor(riskValue);
            
            // Get pixel position - scale to 0-100 for viewBox
            const absoluteX = ((coords.lng + 180) / 360) * 100;
            const absoluteY = ((90 - coords.lat) / 180) * 50;
            
            // Circle radius scaled to viewBox (0.5-2.5% of width)
            const radius = 0.5 + (riskValue * 2);
            
            return (
              <g key={item.country} filter="url(#shadowFilter)">
                {/* Outer glow circle */}
                <circle
                  cx={absoluteX}
                  cy={absoluteY}
      ...
      {/* Legend */}
      <div className="mt-6 grid grid-cols-5 gap-3 text-center w-full max-w-2xl">
        {[
          { risk: 0.15, label: 'Safe', color: '#10B981' },
          { risk: 0.35, label: 'Low', color: '#84CC16' },
          { risk: 0.55, label: 'Medium', color: '#FBBF24' },
          { risk: 0.75, label: 'High', color: '#EA580C' },
          { risk: 0.95, label: 'Critical', color: '#DC2626' }
        ].map((item) => (
          <div key={item.label} className="flex flex-col items-center">
            <div
              className="w-6 h-6 rounded-full mb-2 shadow-lg"
...
// --- COMPONENT: WORLD HEATMAP MAP ---
const WorldHeatmapMap = ({ heatmap }) => {
  // Country coordinates (latitude, longitude) for positioning circles
  const countryCoords = {
    'USA': { lat: 37.0902, lng: -95.7129 },
    'UK': { lat: 55.3781, lng: -3.4360 },
    'Russia': { lat: 61.5240, lng: 105.3188 },
  ...
  const getRiskColor = (risk) => {
    ...
    if (risk > 0.4) return '#FBBF24'; // Yellow
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-slate-950 p-4 rounded-lg">
      <svg width="100%" height="100%" viewBox="0 0 800 400" className="border border-slate-700 rounded bg-gradient-to-br from-blue-900/10 to-slate-950">
        ...
        <g stroke="#334155" strokeWidth="1" fill="none" opacity="0.3">
          {/* North America */}
          <path d="M 150 80 L 200 100 L 200 200 L 150 180 Z"/>
          {/* South America */}
          <path d="M 200 180 L 240 200 L 230 300 L 200 280 Z"/>
          {/* Europe */}
          <path d="M 380 100 L 420 100 L 420 150 L 380 150 Z"/>
          {/* Africa */}
          <path d="M 420 150 L 480 150 L 480 300 L 420 300 Z"/>
          {/* Asia */}
          <path d="M 480 80 L 600 80 L 620 200 L 520 220 Z"/>
          {/* Australia */}
          <path d="M 620 300 L 650 300 L 660 330 L 630 330 Z"/>
        </g>

        {/* Risk circles for each country */}
        {heatmap && heatmap.length > 0 && heatmap.map((item) => {
          const coords = countryCoords[item.country];
          if (!coords) return null;
          
          const svg = latLngToSVG(coords.lat, coords.lng);
          const riskRadius = 8 + (item.risk * 15); // Size based on risk
          const color = getRiskColor(item.risk);

          return (
            <g key={item.country} filter="url(#glow)">
              {/* Outer glow ring */}
              <circle
                cx={svg.x}
                cy={svg.y}
        ...
        <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 min-h-[320px]">
          ...
          {heatmap && heatmap.length > 0 ? (
            <div className="space-y-6">
              ...
              <div className="bg-slate-950 rounded-lg border border-slate-700 p-4 min-h-[420px]">
                <p className="text-xs text-slate-500 font-semibold uppercase mb-4">Global Risk Distribution - Interactive Globe</p>
                <GlobeHeatmapVisualization heatmap={heatmap} />
                <p className="text-xs text-slate-600 mt-3 text-center">
                  Circle size and color indicate risk level • Larger circles = Higher risk
                </p>
              </div>

              {/* Risk Distribution List */}
              <div className="space-y-3">
              <p className="text-xs text-slate-500 font-semibold uppercase">Regional Risk Breakdown</p>
